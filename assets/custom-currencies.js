document.addEventListener('DOMContentLoaded', function() {
    function initCurrencyConverter() {
        // Check if required Shopify properties are available
        if (typeof Shopify === 'undefined' || typeof Shopify.shop_currency === 'undefined' || typeof Currency === 'undefined') {
            console.error("Shopify or Currency objects are not available.");
            return false;
        }
    ...
    function ensureCurrencyConverterInitialized() {
        if (!initCurrencyConverter()) {
            setTimeout(ensureCurrencyConverterInitialized, 100);
        }
    }

    ensureCurrencyConverterInitialized();
});
